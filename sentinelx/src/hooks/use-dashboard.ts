...
export function useDashboard() {
  ...
  const loadMockData = () => {
    ...
    const mockActivityFeed: ActivityFeed[] = [
      { id: '1', type: 'alert', title: 'Suspicious login attempt detected', severity: 'high', status: 'investigating', timestamp: new Date().toISOString() },
      { id: '2', type: 'incident', title: 'Malware signature identified', severity: 'critical', status: 'mitigated', timestamp: new Date().toISOString() },
      { id: '3', type: 'alert', title: 'Automated response initiated', severity: 'medium', status: 'resolved', timestamp: new Date().toISOString() },
      { id: '4', type: 'incident', title: 'Unusual network traffic pattern', severity: 'medium', status: 'monitoring', timestamp: new Date().toISOString() },
      { id: '5', type: 'alert', title: 'Phishing attempt blocked', severity: 'high', status: 'mitigated', timestamp: new Date().toISOString() },
    ];

    const mockRecentAlerts = [
      { id: 1, title: 'Brute Force Attack Detected', severity: 'high', status: 'mitigated', timestamp: new Date().toISOString(), source: 'Authentication System' },
    ];
  ...
  return {
    ...
    refresh: async () => {
      try {
        ...
        console.error('Dashboard data refresh error (falling back to mock data):', err);
        loadMockData();
        setIsOffline(true);
      } finally {
        setIsLoading(false);
