...
export function WorkflowList({ 
  ...
  const getStatusIcon = (status: WorkflowStatus) => {
    switch (status) {
      ...
      case 'inactive': return <Pause className="h-4 w-4 text-slate-400" />;
    ...
    <div className="space-y-4">
      {workflows.map((workflow) => (
        <div
          key={workflow.id}
          className={cn(
            "glass-neon rounded-xl border border-green-500/30 overflow-hidden transition-all duration-200",
        ...
        >
          ...
          {/* Status Bar */}
          <div className="px-6 py-3 border-b border-green-500/20 bg-black/30">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2">
          ...
          {expandedWorkflow === workflow.id && (
            <div className="p-6 bg-black/20">
                ...
                <div>
                  ...
                  <div className="space-y-2 text-xs">
                    <div className="flex justify-between">
                      <span className="text-slate-400">Auto-trigger:</span>
                      <span className={workflow.configuration.autoTrigger ? "text-green-400" : "text-slate-400"}>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-white">{Math.round(workflow.configuration.timeout / 1000)}s</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-400">Retry Attempts:</span>
                      <span className="text-white">{workflow.configuration.retryAttempts}</span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-2">
                    {workflow.steps.slice(0, 3).map((step, index) => (
                      <div key={step.id} className="flex items-center gap-2 text-xs">
                        ...
                        <span className="text-slate-500">({step.actions.length} actions)</span>
                      </div>
                    ))}
              ...
              {/* Action Buttons */}
              <div className="flex items-center gap-3 mt-6 pt-4 border-t border-green-500/20">
                <button
                ...
                <button
                  onClick={() => onWorkflowEdit(workflow.id)}
                  className="px-4 py-2 bg-black/50 hover:bg-black/70 text-green-400 border border-green-500/30 rounded-lg transition-colors font-medium"
                >
                  Edit Workflow
                </button>
